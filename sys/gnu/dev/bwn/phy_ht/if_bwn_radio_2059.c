/*
  ...
  IEEE 802.11n HT-PHY Radio 2059 support
  ...
  Channel tables extracted from MMIO dump of 6.30.223.141
...
/* Radio init table for PHY rev 1 */
static const uint16_t r2059_phy_rev1_init[][2] = {
...
#define	RADIOREGS(r00, r01, r02, r03, r04, r05, r06, r07, r08, r09, \
	...
	.radio_syn16 = r00, .radio_syn17 = r01, .radio_syn22 = r02, \
	.radio_syn25 = r03, .radio_syn27 = r04, .radio_syn28 = r05, \
	.radio_syn29 = r06, .radio_syn2c = r07, .radio_syn2d = r08, \
	.radio_syn37 = r09, .radio_syn41 = r10, .radio_syn43 = r11, \
	.radio_syn47 = r12, .radio_rxtx4a = r13, .radio_rxtx58 = r14, \
	.radio_rxtx5a = r15, .radio_rxtx6a = r16, .radio_rxtx6d = r17, \
	.radio_rxtx6e = r18, .radio_rxtx92 = r19, .radio_rxtx98 = r20

#define	PHYREGS(r0, r1, r2, r3, r4, r5) \
	.phy_regs = { .bw1 = r0, .bw2 = r1, .bw3 = r2, \
		      .bw4 = r3, .bw5 = r4, .bw6 = r5 }

/* Channel table for 2.4GHz and 5GHz bands */
static const struct bwn_phy_ht_channeltab_e_r2059 bwn_phy_ht_channeltab_r2059[] = {
	/* 2.4 GHz band */
	{
		.freq = 2412,
...
#define	N_CHAN	nitems(bwn_phy_ht_channeltab_r2059)

void
bwn_radio_2059_upload_inittabs(struct bwn_mac *mac)
{
	const uint16_t (*table)[2];
	size_t size, i;

	switch (mac->mac_phy.rev) {
	case 1:
		table = r2059_phy_rev1_init;
		size = nitems(r2059_phy_rev1_init);
		break;
	default:
		device_printf(mac->mac_sc->sc_dev,
		    "HT-PHY: unsupported PHY rev %d for radio 2059\n",
		    mac->mac_phy.rev);
		return;
	}

	for (i = 0; i < size; i++)
		BWN_RF_WRITE(mac, R2059_ALL | table[i][0], table[i][1]);
}
...
bwn_radio_2059_get_channeltab(struct bwn_mac *mac, uint16_t freq)
{
	...
	e = bwn_phy_ht_channeltab_r2059;
	for (i = 0; i < N_CHAN; i++, e++) {
		if (e->freq == freq)
			return e;
	}
...
void
bwn_radio_2059_channel_setup(struct bwn_mac *mac,
    const struct bwn_phy_ht_channeltab_e_r2059 *e)
{
	static const uint16_t routing[] = { R2059_C1, R2059_C2, R2059_C3 };
	unsigned int i;

	/* Write synthesizer registers */
	BWN_RF_WRITE(mac, 0x16, e->radio_syn16);
	BWN_RF_WRITE(mac, 0x17, e->radio_syn17);
	BWN_RF_WRITE(mac, 0x22, e->radio_syn22);
	BWN_RF_WRITE(mac, 0x25, e->radio_syn25);
	BWN_RF_WRITE(mac, 0x27, e->radio_syn27);
	BWN_RF_WRITE(mac, 0x28, e->radio_syn28);
	BWN_RF_WRITE(mac, 0x29, e->radio_syn29);
	BWN_RF_WRITE(mac, 0x2c, e->radio_syn2c);
	BWN_RF_WRITE(mac, 0x2d, e->radio_syn2d);
	BWN_RF_WRITE(mac, 0x37, e->radio_syn37);
	BWN_RF_WRITE(mac, 0x41, e->radio_syn41);
	BWN_RF_WRITE(mac, 0x43, e->radio_syn43);
	BWN_RF_WRITE(mac, 0x47, e->radio_syn47);

	/* Write per-core RX/TX registers */
	for (i = 0; i < 3; i++) {
		BWN_RF_WRITE(mac, routing[i] | 0x4a, e->radio_rxtx4a);
		BWN_RF_WRITE(mac, routing[i] | 0x58, e->radio_rxtx58);
		BWN_RF_WRITE(mac, routing[i] | 0x5a, e->radio_rxtx5a);
		BWN_RF_WRITE(mac, routing[i] | 0x6a, e->radio_rxtx6a);
		BWN_RF_WRITE(mac, routing[i] | 0x6d, e->radio_rxtx6d);
		BWN_RF_WRITE(mac, routing[i] | 0x6e, e->radio_rxtx6e);
		BWN_RF_WRITE(mac, routing[i] | 0x92, e->radio_rxtx92);
		BWN_RF_WRITE(mac, routing[i] | 0x98, e->radio_rxtx98);
	}

	/* Delay for radio to settle */
	DELAY(50);

	/*
	 * Trigger VCO calibration.
	 * Must use mask-before-set pattern and correct bit value (0x4 not 0x40).
	 * From Linux b43 radio_2059.c.
	 */
	BWN_RF_MASK(mac, R2059_RFPLL_MISC_EN, ~0x1);
	BWN_RF_MASK(mac, R2059_RFPLL_MISC_CAL_RESETN, ~0x4);
	BWN_RF_SET(mac, R2059_RFPLL_MISC_CAL_RESETN, 0x4);
	BWN_RF_SET(mac, R2059_RFPLL_MISC_EN, 0x1);
